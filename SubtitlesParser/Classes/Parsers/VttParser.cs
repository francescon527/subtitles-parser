    ...
    private static (WebvttItemDiscriminator Discriminator, object? Parsed) GetNextWebvttElement(TextReader reader)
    {
        ...
        switch (textLineNoCrLf)
        {
            ...
            case { } when !string.IsNullOrWhiteSpace(textLineNoCrLf)
             ...
             return (WebvttItemDiscriminator.Id, textLineNoCrLf);
...
[GeneratedRegex(@"^(?<H1>\d*):?(?<M1>\d\d):(?<S1>\d\d)[,\.]?(?<f1>\d*)\s*-->\s*(?<H2>\d*):?(?<M2>\d\d):(?<S2>\d\d)[,\.]?(?<f2>\d*)(?<settings>(?:\s+\S+)*)$", RegexOptions.Compiled)] 
private static partial Regex GetNonStandardWebvttTimecodeRegex();
    ...
    //     var (tStart, tEnd, settings) = ((TimeSpan, TimeSpan, Dictionary<string,string>))vttElement.Parsed!;
    //     vttBlock.StartTime = tStart;
    //     vttBlock.EndTime = tEnd;

    //     if (settings.Count == 0)
    //          Console.WriteLine("settings: (none)");
    //     else
    //         Console.WriteLine("settings: " + string.Join(", ", settings.Select(kv => $"{kv.Key}:{kv.Value}")));
    ...
    //     for (int i = 0; i < vttBlock.Lines.Length; i++) {
    //         bool hasTagLike = Regex.IsMatch(vttBlock.Lines[i], @"<[^>]+>");
    //         if(hasTagLike) {
    //             vttBlock.Lines[i] = Regex.Replace(vttBlock.Lines[i], "<[^>]*>", string.Empty);
    //         }
    //         Console.WriteLine($"Lines[{i}] = '{vttBlock.Lines[i]}'");
    //     }
    //     vttBlock.PlaintextLines = vttBlock.Lines;
    //     for (int i = 0; i < vttBlock.PlaintextLines.Length; i++) {
    //         bool hasTagLike = Regex.IsMatch(vttBlock.Lines[i], @"<[^>]+>");
    //         if(hasTagLike) {
    //             vttBlock.Lines[i] = Regex.Replace(vttBlock.Lines[i], "<[^>]*>", string.Empty);
    //         }
    //         Console.WriteLine($"PlaintextLines[{i}] = '{vttBlock.PlaintextLines[i]}'");
    //     }
...
private static (bool Resync, SubtitleItem? VttBlock) ParseVttBlock(TextReader vttStream)
{
    ...
    if (vttElement.Discriminator == WebvttItemDiscriminator.Terminated)
        return (Resync: false, VttBlock: null);
    if (vttElement.Discriminator != WebvttItemDiscriminator.Timestamp)
        return (Resync: true, VttBlock: null);
    var (tStart, tEnd, settings) = ((TimeSpan, TimeSpan, Dictionary<string, string>))vttElement.Parsed!;
    vttBlock.StartTime = tStart;
    vttBlock.EndTime = tEnd;
    // settings opzionali, se vuoi tenere il log
    if (settings.Count == 0)
        Console.WriteLine("settings: (none)");
    else
        Console.WriteLine("settings: " + string.Join(", ", settings.Select(kv => $"{kv.Key}:{kv.Value}")));
    ...
    if (vttElement.Discriminator == WebvttItemDiscriminator.Terminated)
        return (Resync: false, VttBlock: null);
    if (vttElement.Discriminator != WebvttItemDiscriminator.Text)
        return (Resync: true, VttBlock: null);
    vttBlock.Lines = (string[])vttElement.Parsed!;
    // pulizia tag (come avevi già)
    for (int i = 0; i < vttBlock.Lines.Length; i++)
    {
        bool hasTagLike = Regex.IsMatch(vttBlock.Lines[i], @"<[^>]+>");
        if (hasTagLike)
    ...
    for (int i = 0; i < vttBlock.PlaintextLines.Length; i++)
    {
        bool hasTagLike = Regex.IsMatch(vttBlock.PlaintextLines[i], @"<[^>]+>");
        if (hasTagLike)
            vttBlock.PlaintextLines[i] = Regex.Replace(vttBlock.PlaintextLines[i], "<[^>]*>", string.Empty);
        Console.WriteLine($"PlaintextLines[{i}] = '{vttBlock.PlaintextLines[i]}'");
    }
    return (Resync: false, vttBlock);
}
...
private static async ValueTask<(bool Resync, SubtitleItem? VttBlock)> ParseVttBlockAsync(TextReader vttStream, CancellationToken cancellation)
{
    ...
    private static (TimeSpan Start, TimeSpan End, Dictionary<string,string> Settings) ParseVttTimecode(Match match)
    {
        ...
        var rawSettings = match.Groups["settings"].Value;
        if (!string.IsNullOrWhiteSpace(rawSettings))
        {
        var tokens = rawSettings.Trim().Split((char[]?)null, StringSplitOptions.RemoveEmptyEntries);
        foreach (var token in tokens)
            ...
            if (key is "align" or "position" or "line" or "size" or "vertical" or "region")
                settings[key] = value;
            }
        }
        return (tStart, tEnd, settings);
    }
